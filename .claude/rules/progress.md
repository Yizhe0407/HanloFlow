...
| 類型 | 數量 |
|------|------|
| 總詞條（active runtime） | 15,895 |
| 人工驗證（trust: human） | ~6,520 |
| base seed（trust: seed，低信任） | source 仍有短詞 active；runtime 以 policy 過濾高風險 seed |
| 最新 round | round148 |

所有迴歸測試全部通過（共 482 筆）：
bus 164、medical 69、transport 60、conversation 34、restaurant 20、shopping 20、hotel 17、taxi 19、bank 21、school 20、family 20、workplace 18
